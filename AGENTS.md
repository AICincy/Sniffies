...
The workbench is a tooling shell only. Its Android application ID is `org.aicincy.sniffies.workbench`, intentionally distinct from the APK's signed package ID. Do not try to ship it as an update to the supplied APK without the original source and signing key.
...
5. The supplied APK's checksum, package identity, version code, version name, SDK levels, Capacitor configuration, and dependency versions are preservation constraints. Do not upgrade, downgrade, replace, or normalize them unless a task identifies a specific defect and authorizes that exact change.
...
- Remediate defects and add features against the preserved application baseline. Do not substitute a newer framework, plugin, SDK, or application version when the original version is unknown.
